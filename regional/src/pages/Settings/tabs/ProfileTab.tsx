...
// frontend. Futuramente serão carregados do perfil do administrador
// regional autenticado pelo backend.
function ProfileTab() {
  const [name, setName] = useState(CURRENT_REGIONAL_ADMIN.name);
  ...
  return (
    <div className="rounded-lg border border-gray-100 bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <FormField
          label="Região"
          value={CURRENT_REGIONAL_ADMIN.region}
          readOnly
          disabled
          className="cursor-not-allowed"
        />
        <FormField label="Cargo" value={CURRENT_REGIONAL_ADMIN.role} readOnly disabled className="cursor-not-allowed" />
      </div>
